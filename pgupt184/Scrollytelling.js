...
const initializeVisualization = () => {
    ...
    // Define extra padding needed for the outermost labels to avoid clipping
    // Increased to 60px for guaranteed clearance, especially for the top axis labels.
    const PADDING_FOR_LABELS = 60; 
    
    // Calculate Radius
    // Use the minimum dimension (width or height) minus the padding for labels
    const minDim = Math.min(
        width - margin.left - margin.right - PADDING_FOR_LABELS * 2, 
        height - margin.top - margin.bottom - PADDING_FOR_LABELS * 2
    );
    
    outerRadius = minDim / 2; // Max available radius, accounting for label space
    ...
    // Center the chart group. Shift by PADDING_FOR_LABELS to ensure everything is visible.
...
const updateVisualization = () => {
    ...
    const maxImpact = 1.0; // The theoretical max impact (100% Emissions)
    ...
    // --- AXIS & GRID LINES (Updated Section) ---
    
    // Define the grid line values (normalized impact scores)
    const gridValues = [0.25, 0.50, 0.75, 1.00];

    // Remove existing grid elements
    chartGroup.selectAll(".radial-grid").remove();
    chartGroup.selectAll(".radial-axis-label").remove(); // Also remove old labels

    // Add concentric circles (Grid Lines)
    chartGroup.selectAll(".radial-grid")
        ...
        .attr("cx", 0)
        .attr("cy", 0)
        .attr("r", d => yRadial(d)) // Use the radial scale to determine radius
        .style("fill", "none")
        .style("stroke", "#4b5563") // Tailwind gray-600 for contrast
        .style("stroke-width", 0.5)
        .style("opacity", 0.6)
        .lower(); // Send circles behind the bars

    // Add Axis Labels (Text for the grid lines - positioned along the vertical axis)
    chartGroup.selectAll(".radial-axis-label")
        .data(gridValues.filter(d => d > 0.1)) // Filter out 0% (inner radius)
        .enter().append("text")
        .attr("class", "radial-axis-label")
        .attr("x", 5) // Offset slightly to the right of the vertical axis
        .attr("y", d => -yRadial(d)) // Position vertically (upwards) at the circle's radius
        .attr("dy", "-0.3em") // Move slightly above the line
        .style("font-size", "10px")
        .style("fill", "#9ca3af") // Tailwind gray-400
        .text(d => d3.format(".0%")(d) + " Max Emissions");
    ...
    bars.enter().append("path")
        ...
        .attr("fill", d => d.color)
        .attr("d", arc)
        // Store initial state for smooth transition
        .each(function(d) { this._current = { impactScore: d3.interpolate(innerRadius, yRadial(d.impactScore))(0) }; }) 
        .merge(bars)
        .transition().duration(1500)
        .attrTween("d", function(d) {
            // Tween the outerRadius from previous state to new state
            const interpolate = d3.interpolate(
                this._current.impactScore, 
                yRadial(d.impactScore) // Target outer radius
            );
            this._current.impactScore = yRadial(d.impactScore); // Update current state
            return function(t) {
                // Return the arc path using the interpolated radius
                return arc({
                    vehicle: d.vehicle,
                    impactScore: d.impactScore, // Use real impact score to keep angle correct
                    outerRadius: interpolate(t) // Use interpolated radius for animation
                });
            };
        })
        .attr("d", arc); // Final attribute setting ensures the full shape is drawn
    ...
    labels.enter().append("text")
        ...
        .attr("transform", d => {
            // Position the label slightly outside the final bar end
            const angle = getLabelAngle(d) - Math.PI / 2;
    ...
    chartGroup.append("text")
        ...
        .attr("y", 30)
        .text("Higher Emissions than ICE"); // Updated center text
...
const setupScrollHandler = () => {
    ...
    // Reset active class across all stories and ensure all headings are visible first
    d3.selectAll('.story-sections .step').classed('is-active', false);
    d3.selectAll('.story-sections .step h2').style('opacity', '1.0').style('transition', 'opacity 0.3s');
    ...
    observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            ...
            if (entry.isIntersecting) {
                ...
                const currentHeading = entry.target.querySelector('h2');
                ...
                if (currentStory === 'critical' && stepIndex === TARGET_STEP_INDEX) {
                    ...
                    // Make the narrative heading transparent when the visualization is active
                    if (currentHeading) {
                        currentHeading.style.opacity = '0.0'; 
                    }

                } else {
                    graphicCol.classList.add('is-hidden');
                    // Ensure heading opacity is restored for non-visualization steps
                    if (currentHeading) {
                         currentHeading.style.opacity = '1.0';
                    }
                }
                 ...
                 graphicCol.classList.add('is-hidden');
                 // Also ensure the heading is visible again when scrolling away
                 const targetHeading = entry.target.querySelector('h2');
                 if (targetHeading) {
                     targetHeading.style.opacity = '1.0';
                 }
            }
        });
    }, {
